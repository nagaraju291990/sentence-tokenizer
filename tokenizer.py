...
def acr_func(file):
	global count
	fp1 = open(file) # Open file on read mode -- input file
	...
	for line in lines:
		...
		log.logging.info("Saving accronym hash key=|%s|, value=|%s|" %(line, "____ACRHASH____" + str(count) + "____ACRHASH____" ))
		count = count + 1
...
if(not file_exists):
	print("Given %s file does not exists" %(inputfile))
	exit()
else:
	#open file using open file mode
	log.logging.info("got input file")
	fp = open(inputfile) # Open file on read mode -- input file
...
out_str = re.sub(r'____ORIGINALLINEBREAK____', '\n', out_str)
print(out_str)